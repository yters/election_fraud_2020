...
plot_graph <- function(year, votes, registrations, fit) {
    for(i in 1:98) {
        ...
        pred <- unlist(fitted(fit)[1:130]*registrations[i,1:130])
...
fit_2020 <- fit_model(votes_2020, registrations_2020, 6)
fit_2016 <- fit_model(votes_2016, registrations_2016, 6)
fit_2012 <- fit_model(votes_2012, registrations_2012, 6)
